...
describe.skipIf(!runtimeClient)("seeded development database", () => {
  it("reads the expected development fixtures without mutating data", async () => {
    ...
    expect(messages.map((message) => message.visibility).sort()).toStrictEqual([
      "INTERNAL",
      "PUBLIC",
    ]);
  ...
  it("reads the applied sequence, constraint, and append-only triggers", async () => {
    const sequence = await runtimeClient!.$queryRaw<Array<{ exists: boolean }>>`
